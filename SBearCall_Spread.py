...
token_id = "Enter Token Id from Broker"
...
while not entry_done:
    ...
    for stock in watchlist:
       ...
       #otm_ce_name, otm_pe_name, ce_otm_strike, pe_otm_strike = tsl.OTM_Strike_Selection('NIFTY', 0, 1)
        ...
        print("===================================================================")
        print(f"Primary ATM CALL Option (Sell Leg): {ATM_CE_symbol_name}")
        print(f"Hedge CALL Option (Buy Leg): {otm_H_ce_name}")
        print("===================================================================")
        ...
        if current_close < first_candle_low:
            ...
            lot_size = tsl.get_lot_size(ATM_CE_symbol_name)

            # Hedge BUY (deeper OTM CALL)
            ltp_hedge = tsl.get_ltp_data(names=otm_H_ce_name)
            #tsl.order_placement(otm_H_ce_name, 'NFO', lot_size, 0, 0, 'MARKET', 'BUY', 'MIS')
            print(f"Paper trade Hedge BUY executed: {otm_H_ce_name} at price: {ltp_hedge}")
            

            # Primary SELL (near OTM CALL)
            ltp_primary = tsl.get_ltp_data(names=ATM_CE_symbol_name)
            #tsl.order_placement(ATM_CE_symbol_name, 'NFO', lot_size, 0, 0, 'MARKET', 'SELL', 'MIS')
            print(f"Paper Trade Primary position SELL executed: {ATM_CE_symbol_name} at price: {ltp_primary}")
            ...
            if enable_sl == 'y':
                ...
                while True:
                    ...
                    try:
                        chart_15_sl = tsl.get_intraday_data(ATM_CE_symbol_name, "NFO", 15)
                        ...
                        print(f"[SL Check {datetime.datetime.now().strftime('%H:%M:%S')}] {ATM_CE_symbol_name} | "
                        ...
                        if last_close > first_candle_low:
                            ...
                            #tsl.order_placement(ATM_CE_symbol_name, 'NFO', lot_size, 0, 0, 'MARKET', 'BUY', 'MIS')
                            print(f"Exited primary position: {ATM_CE_symbol_name}")

                            # Exit hedge BUY next
                            #tsl.order_placement(otm_H_ce_name, 'NFO', lot_size, 0, 0, 'MARKET', 'SELL', 'MIS')
                            print(f"Exited hedge position: {otm_H_ce_name}")
